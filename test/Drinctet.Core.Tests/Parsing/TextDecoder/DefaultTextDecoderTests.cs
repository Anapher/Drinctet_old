...
namespace Drinctet.Core.Tests.Parsing.TextDecoder
{
    public class DefaultTextDecoderTests
    {
        ...
        {
            var result = Core.Parsing.TextDecoder.SpanTextDecoder.SplitQuoted(source.AsSpan(), ',').ToArray();
        ...
        {
            Assert.ThrowsAny<ArgumentException>(() => Core.Parsing.TextDecoder.SpanTextDecoder.SplitQuoted(source, ','));
        }
        ...
        {
            var result = Core.Parsing.TextDecoder.SpanTextDecoder.ParseNumberArray(source);
        ...
        {
            Assert.ThrowsAny<ArgumentException>(() => Core.Parsing.TextDecoder.SpanTextDecoder.ParseNumberArray(source));
        }
        ...
        {
            var result = Core.Parsing.TextDecoder.SpanTextDecoder.ParseGenderSelectionFragment(source.AsSpan(1, source.Length - 2));
        ...
        {
            var result = Core.Parsing.TextDecoder.SpanTextDecoder.ParseRandomSelectionFragment(source);
            CompareFragments(expected, result);
        }
        ...
        {
            var result = Core.Parsing.TextDecoder.SpanTextDecoder.ParseVariableFragment(source);
            CompareFragments(expected, result);
        }
        ...
        {
            var decoder = new Core.Parsing.TextDecoder.SpanTextDecoder();
